...
int ten_plus(const Ten *P, const Ten *R, /**/ Ten *T) {
    const real *p, *r;
    real *t;
    p = P->t; r = R->t; t = T->t;
...
int ten_minus(const Ten *P, const Ten *R, /**/ Ten *T) {
    const real *p, *r;
    real *t;
    p = P->t; r = R->t; t = T->t;
    t[XX] = p[XX] - r[XX]; t[XY] = p[XY] - r[XY]; t[XZ] = p[XZ] - r[XZ];
...
int ten_scalar(const Ten *P, real s, /**/ Ten *T) {
    const real *p;
    real *t;
    p = P->t; t = T->t;
...
int ten_vec(const Ten *T, const real a[3], /**/ real b[3]) {
    const real *t;
    t = T->t;
...
int ten_mult_left(const Ten *R, /*io*/ Ten *T) {
    Ten A;
    const real *r;
    real *t, *a;
    r = R->t; t = T->t; a = A.t;
...
int ten_copy(const Ten *P, /**/ Ten *T) {
    const real *p;
    real *t;
    p = P->t; t = T->t;
