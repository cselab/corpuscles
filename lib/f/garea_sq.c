...
int he_f_garea_sq_ini(real K, He *he, T **pq) {
    T *q;
    int n;
    MALLOC(1, &q);
    n = he_nt(he);
...
int
he_f_garea_sq_argv(char ***p, He *he, T **pq)
{
    int status;
    real x;
    if ((status = argv_real(p, &x)) != CO_OK)
        return status;
    return he_f_garea_sq_ini(x, he, pq);
}

int
he_f_garea_sq_fin(T *q)
{
    FREE(q->area); FREE(q);
    return CO_OK;
}
...
static
void compute_area(He *he, const real *x, const real *y, const real *z, /**/ real *area)
{
    real a[3], b[3], c[3];
    int n, t;
...
real
he_f_garea_sq_energy(T *q, He *he,
			  const real *x, const real *y, const real *z)
{
    int n;
    real *area, A, K;
