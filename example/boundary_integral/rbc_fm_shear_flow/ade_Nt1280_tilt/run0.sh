...
then
    ...
    if [ ! -d Co0Da${Da1}_lam${lam}_g${gam}_dt${dt}_Kc${Kc} ]; then
	
	mkdir Co0Da${Da1}_lam${lam}_g${gam}_dt${dt}_Kc${Kc}
	
    fi
    
    cd Co0Da${Da1}_lam${lam}_g${gam}_dt${dt}_Kc${Kc}
    
    co.run ../../main garea $A $Kga volume $V $Kv juelicher_xin $Kb $C0 $Kad $DA0D strain $ref_file lim $Ka $mub $a3 $a4 $b1 $b2 cortez_zero $R $D $rho $eta $lambda $gamdot $dt $start $end $freq_out $freq_stat '<' $in_file
