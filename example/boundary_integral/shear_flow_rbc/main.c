...
static Subst *subst;
static real R, D;
static real rho, eta_o, lambda, gamdot;
static int step_start, step_end, freq_out, freq_stat;
static real dt, time_start, time_end;
...
static void usg(void) {
  ...
  fprintf(stderr, "bi R D rho eta_o lamda gamdot\n");
  fprintf(stderr, "step_start step_end dt time_start time_end freq_out freq_stat\n");
...
int main(__UNUSED int argc, char **argv) {
  ...
  real t, time, tt;
  int  s, i;
  real eng, et, ega, ev, eb, ebl, ebn, es, esa, esb;
  char name[99];
  real A0, V0, v0;
  real A, V, v;
  real a, e, reg;
  ...
  fputs("#dt s t A/A0 V/V0 v et ega ev eb ebl ebn es esa esb\n", fm);
  fclose(fm);
  ...
  tt = time_start+ (step_end-step_start) * dt;
  if ( time_end < tt ) {
    time_end = tt;
  }
  ...
  fprintf(fm, "R D rho eta_o lambda gamdot = %g %g %g %g %g %g\n", R, D, rho, eta_o, lambda, gamdot);
  fprintf(fm, "step_start step_end dt time_start time_end freq_out freq_stat = %i %i %g %g %g %i %i\n",	
	  step_start, step_end, dt, time_start, time_end, freq_out, freq_stat);
  ...
  while ( 1 ) {
	...
	//}
	  ...
	  es = eng;
	  he_f_strain_energies(force_pointer(Fo[i]), &esb, &esa);
	}
	...
	MSG("et ega ev eb ebl ebn es esa esb= %g %g %g %g %g %g %g %g %g", et, ega, ev, eb, ebl, ebn, es, esa, esb);
      }
      
      if ( (fm = fopen(file_stat, "a") ) == NULL) {
	ER("Failed to open '%s'", file_stat);
      }
      fprintf(fm, "%g %i %g %g %g %g %g %g %g %g %g %g %g %g %g\n", dt, s, t, A/A0, V/V0, v, et, ega, ev, eb, ebl, ebn, es, esa, esb);
      fclose(fm);
