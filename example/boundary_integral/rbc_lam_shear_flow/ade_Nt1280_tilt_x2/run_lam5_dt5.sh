...
then
    ...
    co.run ../../main volume $V $Kv garea $A $Kga juelicher_xin $Kb $C0 $Kad $DA0D strain $ref_file lim $Ka $mub $a3 $a4 $b1 $b2 cortez_zero $R $D $rho $eta $lambda $gamdot $dt $start $end $freq_out $freq_stat '<' $in_file
    ...
    do
	num=$i
	bash run_lam5_dt5.sh $num
    done

    for i in `seq 35 5 120`;
    do
	num=$i
	bash run_lam5_dt5.sh $num
    done

    for i in `seq 130 10 200`;
    do
	num=$i
	bash run_lam5_dt5.sh $num
    done

    for i in `seq 250 50 500`;
    do
	num=$i
	bash run_lam5_dt5.sh $num
    done
