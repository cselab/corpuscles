...
then
    ...
    folder_cont2=${folder}_cont2
    folder_cont3=${folder}_cont3r
    
    if [ ! -d $folder_cont3 ]; then
	mkdir $folder_cont3
    fi
    cd $folder_cont3

    in_file=$(ls ../init/*.off | tail -n 1)

    fname=`basename $in_file`
    fnum=${fname%.*}
    start=$(echo $fnum | sed 's/^0*//')

    echo "folder:  "$folder_cont2
    echo "in_file: "$in_file
    echo "start: "$start

    co.run EPSREL=0.1 EPSABS=0.1 ../../../mainr volume $V $Kv garea $A $Kga juelicher_xin $Kb $C0 $Kad $DA0D strain $ref_file lim $Ka $mub $a3 $a4 $b1 $b2 \
	   repel 0.05 1000 cortez_zero $R $D $rho ${eta_out} $lambda $gamdot $dt $start $end $freq_out $freq_stat '<' $in_file '1>/dev/null' '2>/dev/null'
    
else

    num=2000
    bash run_dt50_cont3r.sh $num
