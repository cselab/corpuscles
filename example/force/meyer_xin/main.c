...
static real energy() { return f_meyer_xin_energy(XX, YY, ZZ); }
static int force() {
    zero(NV, FX); zero(NV, FY); zero(NV, FZ);
    return f_meyer_xin_force(XX, YY, ZZ, FX, FY, FZ);
}
static void main0() {
    int i, nstep;
    real e0;
    ...
    f_meyer_xin_energy_ver(&eng);
    f_meyer_xin_area_ver(&area);
    f_meyer_xin_curva_mean_ver(&curva_mean);
    f_meyer_xin_curva_gauss_ver(&curva_gauss);

    f_meyer_xin_laplace_ver(&lbx, &lby, &lbz);
    f_meyer_xin_norm_ver(&nx, &ny, &nz);
...
int main(int __UNUSED argc, const char *v[]) {
  ...
  arg();
  ini("/dev/stdin");
  f_meyer_xin_ini(Kb, C0, Kad, DA0D);
