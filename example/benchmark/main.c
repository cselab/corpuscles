...
static int
tensor_ini(int n, real ** xx, real ** xy, real ** xz, real ** yy,
           real ** yz, real ** zz)
{
    matrix_ini(n, n, xx);
    matrix_ini(n, n, xy);
...
static int
tensor_fin(real * xx, real * xy, real * xz, real * yy, real * yz,
           real * zz)
{
    matrix_fin(xx);
    matrix_fin(xy);
...
static int
vector_tensor(int n, real s, const real * x, const real * y,
              const real * z, real * Txx, real * Txy, real * Txz,
              real * Tyy, real * Tyz, real * Tzz, real * u, real * v,
              real * w)
{
    int i;
    ...
    if (1 - lambda > tol || 1 - lambda < -tol) {
        ...
        for (k = 1; k <= iter_max; k++) {
            ...
            if (ratio < tol) {
                ...
                if (k == iter_max) {
                    MSG("t d dd ratio k = %f %f %f %f %i", t, d, dd, ratio,
                        k);
    ...
    while (1) {
        ...
        if (s % freq_stat == 0) {
            ...
            if (s / 10 % freq_stat == 0) {
                ...
                MSG("et ega ev eb ebl ebn es = %f %f %f %f %f %f %f", et,
