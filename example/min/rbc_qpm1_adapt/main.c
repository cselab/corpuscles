...
//
//One constratint on reduced volume is implemented
...
static real A, V, vc;
static real ea, ev, erv, eb, ebl, ebn, es;

static real *x, *y, *z;
static int Nt, Ne, Nv;
static int *D0, *D1, *D2, *D3;

static real ca, cv, crv;
...
static void init() {
    ...
    force_argv("rvolume", &argv, he, &force_rvolume);
...
real ConstraintRvolume(const real *x, const real *y, const real *z) {
  
  real c;

  vc  = force_energy(force_rvolume, he, x, y, z);
  c   = (vc-v);
  crv = c;
...
real EnergyRvolume(const real *x, const real *y, const real *z) {

  real c, e;
  
  c  = ConstraintRvolume(x, y, z);
  e  = c*c;
  e *= Kc/2.0;
...
void ForceRvolume(const real *x, const real *y, const real *z,
                 /**/ real *fx, real *fy, real *fz) {
    ...
    MALLOC(Nv, &frvx); MALLOC(Nv, &frvy); MALLOC(Nv, &frvz);
    zero(Nv, frvx); zero(Nv, frvy); zero(Nv, frvz);

    force_force(force_rvolume, he, x, y, z, /**/ frvx, frvy, frvz);

    crv = ConstraintRvolume(x, y, z);
...
static int main0(real *vx, real *vy, real *vz,
	...
	off_he_xyz_write(he, x, y, z, file);
	    ...
	    vc = force_energy(force_rvolume, he, x, y, z);
            ...
            MSG("v, ca, cv, crv, Kc: %g %g %g %g %g", vc, ca, cv, crv, Kc);
            MSG("et, ea, ev, erv, ek, eb, ebl, ebn, es: %g %g %g %g %g %g %g %g %g", et, ea, ev, erv, ek, eb, ebl, ebn, es);

            fm = fopen(fullpath(filemsg), "a");
            static int First = 1;
            if (First) {
                fputs("dt s t ca cv crv vc, et ea ev erv ek eb ebl ebn es\n", fm);
                First = 0;
            }
            fprintf(fm, "%g %d %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n",
                    dt, i, time, ca, cv, crv, vc, Kc, et, ea, ev, erv, ek, eb, ebl, ebn, es);
            fclose(fm);
