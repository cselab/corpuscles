...
/********** print a little header information */
    ...
    fprintf(stderr, "\nXYZT dimensions: %d %d %d %d", hdr.dim[1],
            hdr.dim[2], hdr.dim[3], hdr.dim[4]);
    fprintf(stderr, "\nDatatype code and bits/pixel: %d %d", hdr.datatype,
            hdr.bitpix);
...
/********** allocate buffer and read first 3D volume from data file */
    data =
        (MY_DATATYPE *) malloc(sizeof(MY_DATATYPE) * hdr.dim[1] *
                               hdr.dim[2] * hdr.dim[3]);
    if (data == NULL) {
    ...
    ret =
        fread(data, sizeof(MY_DATATYPE),
              hdr.dim[1] * hdr.dim[2] * hdr.dim[3], fp);
    if (ret != hdr.dim[1] * hdr.dim[2] * hdr.dim[3]) {
        fprintf(stderr, "ret: %d\n", ret);
        fprintf(stderr, "sizeof(MY_DATATYPE): %d\n", sizeof(MY_DATATYPE));
...
/********** if nii, write extender pad and image data   */
    if (do_nii == 1) {
        ...
        ret =
            fwrite(data, (size_t) (hdr.bitpix / 8),
